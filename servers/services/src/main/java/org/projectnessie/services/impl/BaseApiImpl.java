...
abstract class BaseApiImpl {
  ...
  WithHash<NamedRef> namedRefWithHashOrThrow(@Nullable String namedRef, @Nullable String hashOnRef, @Nullable
    ...
    WithHash<NamedRef> namedRefWithHash;
    try {
      WithHash<Ref> refWithHash;
      if (params.isFetchUnreachableCommits()) {
        refWithHash = getStore().toRef(namedRef, GetNamedRefsParams.UNREACHABLE_DEFAULT);
      } else {
        refWithHash = getStore().toRef(namedRef);
      }
      Ref ref = refWithHash.getValue();
      if (!(ref instanceof NamedRef)) {
        throw new ReferenceNotFoundException(
