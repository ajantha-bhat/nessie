...
public final class GCMetadataUtil {
  ...
  public static List<String> computeAllFiles(Snapshot snapshot, FileIO io) {
    ...
    try {
      ...
      // Checkpoint info in IdentifiedResultsRepo per reference is the commit hash of the
      // last live commit in that reference (traversing from the head).
      // Traversing commits from head to checkpoint can contain few expired commits in-between,
      // and it will be collected for GC again in the next run.
      // But the manifestList, manifest and data files of those contents might have
