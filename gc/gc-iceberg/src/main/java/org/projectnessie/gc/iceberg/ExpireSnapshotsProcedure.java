...
public class ExpireSnapshotsProcedure extends BaseGcProcedure {
  ...
  public InternalRow[] call(InternalRow internalRow) {
    ...
    if (runId == null) {
      String latestCompletedRunID = identifiedResultsRepo.getLatestCompletedRunID();
      ...
      expiredSnapshotsPerContentId
          ...
          .forEach(
              entry -> {
                ...
                Row row = identifiedResultsRepo.getAnyCommitHashAndKeyForContentId(contentId);
                String commitHash = row.getString(0);
                Content content = getContent(api, commitHash, contentId);
                // keep content key same as content id with dummy namespace db1
                String headCommitHash = getHeadCommitHash(api, refName);
                CommitMeta commitMeta =
                    CommitMeta.builder().message("putting the contents for GC").build();
                try {
                  api.commitMultipleOperations()
                      .branch(Branch.of(refName, headCommitHash))
                      .commitMeta(commitMeta)
                      .operation(
                          Operation.Put.of(ContentKey.of("db1", contentId), content, content))
                      .commit();
                } catch (NessieNotFoundException | NessieConflictException e) {
                  throw new RuntimeException(e);
                }
                List<String> filesToBeDeleted = new ArrayList<>();
                Table table = nessieCatalog.loadTable(TableIdentifier.parse("db1." + contentId));
  ...
  private Content getContent(NessieApiV1 api, String hash, String contentId) {
    try {
