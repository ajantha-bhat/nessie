...
public class ExpireSnapshotsProcedure extends AbstractGcProcedure {
  ...
  public InternalRow[] call(InternalRow internalRow) {
    ...
    Map<String, ContentAndSnapshots> content = new HashMap<>();
      ...
      repo.collectExpireableSnapshots(identifyRuns)
          ...
          .forEach(
              row -> {
                ...
                String expiredSnapshotIds = row.getString(1);
                Set<Long> expiredSnapshots = new HashSet<>();
                if (!expiredSnapshotIds.isEmpty()) {
                  Arrays.stream(expiredSnapshotIds.split(" "))
                      .mapToLong(Long::parseLong)
                      .forEach(expiredSnapshots::add);
                }
                content.compute(
                    contentId,
                    (cid, existing) -> {
                      if (existing != null) {
                        existing.expiredSnapshotIds.addAll(expiredSnapshots);
                        return existing;
                      } else {
                        String tableIdent = row.getString(2);
                        return new ContentAndSnapshots(contentId, expiredSnapshots, tableIdent);
                      }
                    });
              });
    }
    ...
    try (NessieCatalog nessieCatalog = createNessieGcCatalog()) {
      ...
      for (ContentAndSnapshots cmp : content.values()) {
        ...
        TableInformation tableInformation = new TableInformation(cmp.contentId, cmp.tableIdent);
  ...
  static final class TableInformation {
    ...
    TableInformation(String contentId, String tableName) {
    ...
    public String toString() {
      return String.format(
          "content-id %s expired for '%s' , contentId, tableName", contentId, tableName);
    }
  }
  ...
  private void expireForTable(
    ...
    String tableRef = String.format("%s@%s#%s", tableInformation.tableName, expireBranch, null);
    ...
    if (expireSnapshotIds.isEmpty()) {
      ...
      result.accept(
          resultRow(
              timestamp, tableInformation, true, STATUS_NO_EXPIRE, null, false, null, null, null));
      return;
    }
