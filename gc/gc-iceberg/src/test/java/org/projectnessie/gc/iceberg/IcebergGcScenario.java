        ...
        .allSatisfy(
            ec -> {
              ...
              assertThat(gcRecord)
                  ...
                  .extracting(
                      gcr -> {
                        String liveSnapshotIds = gcr.getExpiredSnapshotIds();
                        long count = 0;
                        if (liveSnapshotIds != null && !liveSnapshotIds.isEmpty()) {
                          count = Arrays.stream(liveSnapshotIds.split(" ")).count();
                        }
                        return count;
                      },
                      IcebergGcRecord::getTableIdentifier)
                  .containsExactly(tuple(ec.numLiveSnapshots, ec.tableName));
              // TODO: check collected snapshot ?
            });
  }
